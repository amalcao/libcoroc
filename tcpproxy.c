...
int main (int argc, char **argv)
{
    ...
    while ((cfd = tsc_net_accept(fd, remote, &rport)) >= 0) {
        ...
        thread_allocate (proxy_task, cfd, "proxy", TSC_THREAD_NORMAL, 0);
    }
...
int proxy_task (void *v)
{
    ...
    thread_allocate (rwtask, mkfd2(fd, remotefd), "rwtask", TSC_THREAD_NORMAL, 0);
    thread_allocate (rwtask, mkfd2(remotefd, fd), "rwtask", TSC_THREAD_NORMAL, 0);
