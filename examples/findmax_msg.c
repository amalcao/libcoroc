...
int find_max (void * arg)
{
	...
	tsc_recv (&size,sizeof(int),  true);
	if (size <= 0) tsc_coroutine_exit (-1);

	buf = malloc (size * sizeof(int));
	tsc_recv ( buf,size * sizeof(int), true);
...
int main (void * arg)
{
	...
	tsc_send (slave, &size, sizeof(int)); // tsc_send the size of array
