...
static int __tsc_parse_message (struct tsc_client_param *param) {
    ...
    while (tsc_net_read(param->socket, &len, sizeof(int32_t)) > 0) {
        ...
        if (tsc_chan_send(param->comm, &message) == CHAN_CLOSED)
            break;
    }
...
static int __tsc_server_loop (tsc_service_t *service) {
    ...
    for (;;) {
        ...
        tsc_coroutine_spawn(__tsc_parse_message,
            param, "parsing");
    }
...
static int __tsc_client_loop (struct tsc_client_param *param) {
    ...
    for (;;) {
        ...
        if (message == NULL) {
            ...
            assert (tsc_net_write(param->socket, &(message->len), sizeof(uint32_t)) == sizeof(uint32_t));
            // and then, sending the content of this message
            assert (tsc_net_write(param->socket, message->buf, message->len) == message->len);
...
int tsc_service_start (tsc_service_t *service, bool backend) {
    ...
    service->comm = tsc_refcnt_get(
                                   tsc_chan_allocate(sizeof(void*), 0));
    // create the channel for quiting ..
    service->quit = tsc_refcnt_get(
                                   tsc_chan_allocate(sizeof(int), 1));

    // create a coroutine to establish the TCP server..
    if (service->stype & TSC_SERVICE_REMOTE_TCP)
      tsc_coroutine_spawn(__tsc_server_loop,
                          (void*)service, "tcp");

    // create a coroutine to service the message channel..
    if (backend) {
        tsc_coroutine_spawn(__tsc_message_pump, 
                            (void*)service, "pump");
    } else {
...
TSC_SERVICE_DECLARE_LOCAL(__dummy_service, 
    "dummy", TSC_SERVICE_INFINITY,
...
int tsc_init_all_services (void) {
    ...
    for (; ser < TSC_SERVICE_END(); ++ser, ++id) {
        ...
        if ((*ser)->init_callback)
          (*ser)->init_callback(*ser, &id);
        if ((*ser)->auto_start)
          tsc_service_start(*ser, true);
    }
