...
static inline void *atomic_queue_try_rem(queue_t *queue) {
  ...
  if (!queue->status || lock_try_acquire(&queue->lock)) return NULL;
...
static inline void atomic_queue_walk(queue_t *queue,
                                     void (*inspector)(void *)) {
  if (queue->status) {
    ;
    lock_acquire(&queue->lock);
...
static bool general_inspector(void *p0, void *p1) { return p0 == p1; }
