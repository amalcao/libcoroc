...
void mult_Atv (double *v, double *u)
{
	...
	_v = v; _u = u;
	for (k = 0; k < NumCPU; k++) {
		thread_t task = thread_allocate (task_Atv, k, "", TSC_THREAD_NORMAL, NULL);		
	}

	for (k = 0; k < NumCPU; k++)
		channel_recv (finishChan, & finish);
}
...
void mult_Av (double *v, double *u)
{
	...
	_v = v; _u = u;
	for (k = 0; k < NumCPU; k++) {
		thread_t task = thread_allocate (task_Av, k, "", TSC_THREAD_NORMAL, NULL);		
	}
...
void user_main (int argc, char ** argv)
{
	...
	finishChan = channel_allocate (sizeof(bool), 0);
	printf ("%0.9f\n", SpectralNorm(Num));
	//channel_dealloc (finishChan);
}
